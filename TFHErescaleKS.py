#!/bin/python3

import  numpy as np
from scipy.special import erfc
import gmpy2
from gmpy2 import mpfr

gmpy2.get_context().precision=200

# 128bit TFHE's parameter.
lvl0_μ = lvl1_μ = 2**29;
lvl2_μ = 2**61;

class lvl0param:
    n = 634
    α = 2**-15
    st = 2

class lvl1param:
    nbit = 10
    n = 2**nbit
    st = 2
    l = 3
    Bgbit = 7
    Bg = 2**Bgbit
    α = 2**-25
    ε = 1/(2*(Bg**l))
    β = Bg/2

class lvl2param:
    nbit = 11
    n = 2**nbit
    st = 2
    l = 4
    Bgbit = 9
    Bg = 2**Bgbit
    α = 2**-44
    ε = 1/(2*(Bg**l))
    β = Bg/2

class lvl10param:
    t = 8
    basebit = 2
    scalebasebit = 32

class lvl21param:
    t = 8
    basebit = 4
    scalebasebit = 64

class lvl22param:
    t = 8
    basebit = 4

class lvl20param:
    t = 7
    basebit =  2

def cmuxnoisecalc(P,α):
    return 2*P.l*P.n*(P.β**2)*(α**2)+(1+P.n)*(P.ε**2)

def brnoisecalc(lowP,highP):
    return lowP.n*cmuxnoisecalc(highP,highP.α)

def iknoisecalc(domainP,targetP,funcP):
    return domainP.n*(2**(-2*(funcP.basebit*funcP.t+1)))+funcP.t*domainP.n*(targetP.α**2)

def rescaleiknoisecalc(domainP,targetP,funcP):
    return domainP.n*(2**(-2*(funcP.scalebasebit+1)))+domainP.n*((targetP.st*targetP.α)**2)

def gbnoisecalc(lowP,highP,funcP):
    return brnoisecalc(lowP,highP)+iknoisecalc(highP,lowP,funcP)

def rescalegbnoisecalc(lowP,highP,funcP):
    return brnoisecalc(lowP,highP)+rescaleiknoisecalc(highP,lowP,funcP)

print(iknoisecalc(lvl1param,lvl0param,lvl10param))
print(rescaleiknoisecalc(lvl1param,lvl0param,lvl10param))
print(gbnoisecalc(lvl0param,lvl1param,lvl10param))
print(rescalegbnoisecalc(lvl0param,lvl1param,lvl10param))

def privksnoisecalc(domainP,targetP,privksP):
    return domainP.n*(2**(-2*(privksP.basebit*privksP.t+1)))+privksP.t*domainP.n*(targetP.α**2)

def rescaleprivksnoisecalc(domainP,targetP,privksP):
    return domainP.n*(2**(-2*(privksP.scalebasebit+1)))+domainP.n*((targetP.st*targetP.α)**2)

def cbnoisecalc(domainP,middleP,targetP,privksP):
    return brnoisecalc(domainP,middleP)+privksnoisecalc(middleP,targetP,privksP)

def rescalecbnoisecalc(domainP,middleP,targetP,privksP):
    return brnoisecalc(domainP,middleP)+rescaleprivksnoisecalc(middleP,targetP,privksP)

print(privksnoisecalc(lvl2param,lvl1param,lvl21param))
print(rescaleprivksnoisecalc(lvl2param,lvl1param,lvl21param))
print(cbnoisecalc(lvl0param,lvl2param,lvl1param,lvl21param))
print(rescalecbnoisecalc(lvl0param,lvl2param,lvl1param,lvl21param))

# Processor parameter

# ROMaddress = 7 # 4 word block
# RAMaddress = 9
# RAMwordbit = 8

# print("TFHE Gate Bootstrapping Noise")

# gbnoise  = gbnoisecalc(lvl0param,lvl1param,lvl10param)

# print(gbnoise)

# print("TFHE GB error prob")
# print(erfc(1/(16*np.sqrt(2*gbnoise))))

# cbnoise = cbnoisecalc(lvl0param,lvl2param,lvl2param,lvl22param)

# print("TFHE lvl20 iks noise")
# print(iknoisecalc(lvl0param,lvl2param,lvl20param))

# Processor parameter

# ROMaddress = 7 # 4 word block
# RAMaddress = 9
# RAMwordbit = 8

# print("TFHE Gate Bootstrapping Noise")

# gbnoise  = gbnoisecalc(lvl0param,lvl1param,lvl10param)

# print(gbnoise)

# print("TFHE GB error prob")
# print(erfc(1/(16*np.sqrt(2*gbnoise))))

# cbnoise = cbnoisecalc(lvl0param,lvl2param,lvl2param,lvl22param)

# print("TFHE lvl20 iks noise")
# print(iknoisecalc(lvl0param,lvl2param,lvl20param))

# print("TFHE Circuit Bootstrapping Noise")
# print(cbnoise)

# def romnoisecalc(addressP,dataP,middleP,ikP,privksP,ROMaddress):
#     return dataP.α+ROMaddress*cmuxnoisecalc(dataP,cbnoisecalc(addressP,middleP,dataP,privksP))+iknoisecalc(addressP,dataP,ikP)

# print("TFHE ROM CMUX noise")
# romnoise = romnoisecalc(lvl0param,lvl2param,lvl2param,lvl20param,lvl22param,ROMaddress)
# print(romnoise)

# print("TFHE ROM error prob")
# print(erfc(1/(16*np.sqrt(2*romnoise))))

# def ramnoisecalc(addressP,dataP,middleP,ikP,privksP,RAMaddress):
#     return brnoisecalc(addressP,dataP)+RAMaddress*cmuxnoisecalc(dataP,cbnoisecalc(addressP,middleP,dataP,privksP))+iknoisecalc(addressP,dataP,ikP)

# print("RAM Read Noise")
# rnoise = ramnoisecalc(lvl0param,lvl2param,lvl2param,lvl20param,lvl22param,RAMaddress)
# print(rnoise)
# print("RAM Read error prob")
# print(erfc(1/(16*np.sqrt(2*rnoise))))
# print("RAM Read error prob in 3000 cycle")
# print(1-(1-mpfr(erfc(1/(16*np.sqrt(2*rnoise)))))**(3000*RAMwordbit))

# print("Packing Switch noise")


# psnoise = DEF_n*2*DEF_l*DEF_N*(DEF_β**2)*(DEF_αbk**2)+DEF_n*(1+DEF_N)*(DEF_ε**2) + DEF_N*(2**(-2*(DEF_basebit*DEF_t+1)))+8*DEF_t*DEF_N*(DEF_αbk**2)
# print(psnoise)

# print("significant term")
# print(DEF_n*2*DEF_lbar*DEF_nbar*(DEF_βbar**2)*(DEF_αbklvl02**2))
# print(DEF_n*(1+DEF_nbar)*(DEF_εbar**2))
# print(DEF_nbar*(2**(-2*(DEF_basebitlvl21*DEF_tbar+1))))
# print(DEF_tbar*DEF_nbar*(DEF_αprivks**2))
